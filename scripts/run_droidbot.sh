...
result_dir=$OUTPUT_DIR/$apk_file_name.droidbot.dfs.greedy.result.$AVD_SERIAL.$AVD_NAME\#$current_date_time
mkdir -p $result_dir
...
# run DroidBot
echo "** RUN DroidBot (dfs-greedy) (${AVD_SERIAL})"
adb -s $AVD_SERIAL shell date "+%Y-%m-%d-%H:%M:%S" >> $result_dir/droidbot_testing_time_on_emulator.txt
if [[ $LOGIN_SCRIPT != "" ]]
then
    timeout $TEST_TIME droidbot -d $AVD_SERIAL -a $APK_FILE -o $result_dir -timeout 21600 -count 100000 -keep_app -keep_env -random -policy dfs_greedy -grant_perm -is_emulator 2>&1 | tee $result_dir/droidbot.log
else
    timeout $TEST_TIME droidbot -d $AVD_SERIAL -a $APK_FILE -o $result_dir -timeout 21600 -count 100000 -random -policy dfs_greedy -grant_perm -is_emulator 2>&1 | tee $result_dir/droidbot.log
fi
adb -s $AVD_SERIAL shell date "+%Y-%m-%d-%H:%M:%S" >> $result_dir/droidbot_testing_time_on_emulator.txt
